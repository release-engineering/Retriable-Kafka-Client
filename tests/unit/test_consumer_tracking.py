...
def test_partition_info_round_trip() -> None:
    """Test that _TrackingInfo can be converted to TopicPartition and maintains data integrity."""
    # Create a mock Kafka message and wrap in MessageGroup
    ...
    message_group = MessageGroup(
        topic="test-topic", partition=3, messages={0: mock_message}
    )

    # Convert MessageGroup to TrackingInfo
    partition_info = TrackingInfo.from_message_group(message_group)
...
def test_offset_cache_schedule_commit_success(
    ...
    result = cache.schedule_commit(message_group)
