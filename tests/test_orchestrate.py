...
def test_thread_init(sample_config: TopicConfig):
    consumer = BaseConsumer(config = sample_config, executor=MagicMock(), max_concurrency=1)
...
def test_consume_topics_creates_executor_and_threads(
    ...
    with patch("retriable_kafka_client.consumer.Consumer"), \
        ...
        call_index = 0
        with patch("retriable_kafka_client.orchestrate.BaseConsumer") as mock_consumer_base:
